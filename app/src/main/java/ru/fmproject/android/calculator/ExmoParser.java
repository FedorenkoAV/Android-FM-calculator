 ...
 */
public class ExmoParser extends AsyncTask<String, String, String[]> {

    private XMLPullParser.MyCustomCallBack callback;

    private static final String TAG = "ExmoParser";
    ...
    public ExmoParser(Context context, TextView contentView, final XMLPullParser.MyCustomCallBack callback) {
        super();
    ...
    public ExmoParser(Context context, final XMLPullParser.MyCustomCallBack callback) {
        super();
        this.context = context;
        this.callback = callback;
    }
...
//            L.d(TAG, "LocalizedMessage: " + e.getLocalizedMessage());
        ...
        try {
            ...
            while ((length = inputStream.read(buffer)) != -1) {
                baosResult.write(buffer, 0, length);
            }
            String resultJson = baosResult.toString("UTF-8");

            JSONObject dataJsonObj = null;

            dataJsonObj = new JSONObject(resultJson);
//            L.d(TAG, "dataJsonObj: " + dataJsonObj);
//                JSONArray friends = dataJsonObj.getJSONArray("BTC_USD");
            JSONObject btcToUsdJsonObj = dataJsonObj.getJSONObject("BTC_USD");
            L.d(TAG, "btcToUsdJsonObj: " + btcToUsdJsonObj);
            kurs = btcToUsdJsonObj.getString("avg");
